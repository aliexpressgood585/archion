...
interface Phase { id: string; label: string; pct: number; enabled: boolean }
...
const PROJECT_RATE: Record<string, number> = {
  residential_small: 8, residential_large: 6.5, commercial: 5.5, public: 7, renovation: 9,
}
const PROJECT_LABELS: Record<string, string> = {
  residential_small: 'מגורים — עד 300 מ"ר', residential_large: 'מגורים — מעל 300 מ"ר',
  commercial: 'מסחרי / משרדים', public: 'מבנה ציבורי', renovation: 'שיפוץ / שדרוג',
}

const DEFAULT: State = {
  projectType: 'residential_small',
  constructionCost: '',
  customRate: '',
  phases: BASE_PHASES.map(p => ({ ...p, enabled: true })),
}

export default function FeeCalculator({ projectId }: { projectId: string | null }) {
  const { state, setState, loading, saving } = useToolState('fee-calculator', projectId, DEFAULT)
  const { projectType, constructionCost, customRate, phases } = state

  const togglePhase = (id: string) =>
    setState(s => ({ ...s, phases: s.phases.map(p => p.id === id ? { ...p, enabled: !p.enabled } : p) }))
  ...
  const feePerPhase = phases.map(p => ({ ...p, amount: p.enabled ? (totalFee * p.pct / enabledPctSum) : 0 }))
    ...
    <div className="space-y-6" dir="rtl">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div>
          ...
          <select value={projectType} onChange={e => setState(s => ({ ...s, projectType: e.target.value }))}
        ...
        <div>
          ...
          <input type="text" value={constructionCost}
            onChange={e => setState(s => ({ ...s, constructionCost: e.target.value }))}
        ...
        <div>
          ...
          <input type="number" min="1" max="20" step="0.5" value={customRate}
            onChange={e => setState(s => ({ ...s, customRate: e.target.value }))}
      ...
      <div>
        <div className="text-sm font-medium text-slate-700 mb-2">שלבי שירות</div>
              ...
              {feePerPhase.map((p, i) => (
                <tr key={p.id} className={i % 2 === 0 ? 'bg-white' : 'bg-slate-50/50'}>
                  <td className="px-3 py-2 text-center">
                    <input type="checkbox" checked={p.enabled} onChange={() => togglePhase(p.id)}
